...
type bitmapConflictRange struct {
	DungeonMq int
	StartBit  int
	Vanilla   []string
	Mq        []string
...
func init() {
	...
	addOotSilverRupeeBitmapConflicts()
...
var ootSilverRupeeConflictRanges = []bitmapConflictRange{
	{
		DungeonMq: OotMqTempleSpirit,
		StartBit:  10,
		Vanilla: []string{
	...
	{
		DungeonMq: OotMqGerudoTrainingGrounds,
		StartBit:  65,
		Vanilla: []string{
...
func addOotSilverRupeeBitmapConflicts() {
	conflicts := ootBitmapConflictTable["srOot"]
	for _, group := range ootSilverRupeeConflictRanges {
		addBitmapConflictRange(conflicts, "srOot", group)
	}
}

func addBitmapConflictRange(conflicts map[int]bitmapConflictEntry, block string, group bitmapConflictRange) {
	if len(group.Vanilla) != len(group.Mq) {
		panic(fmt.Sprintf("bitmap conflict range %s bit %d has mismatched variant lengths", block, group.StartBit))
	}
	for index := range group.Vanilla {
		bit := group.StartBit + index
		if _, exists := conflicts[bit]; exists {
			panic(fmt.Sprintf("duplicate bitmap conflict entry for %s bit %d", block, bit))
		}
		conflicts[bit] = bitmapConflictEntry{
			Block:     block,
			Bit:       bit,
			DungeonMq: group.DungeonMq,
			Vanilla:   []string{group.Vanilla[index]},
			Mq:        []string{group.Mq[index]},
		}
	}
}
...
func ootConflictingSilverRupeeCheckNames(oot *OotState, bitPos int) ([]string, bool) {
	return ootConflictingBitmapCheckNames(oot, "srOot", bitPos)
}
